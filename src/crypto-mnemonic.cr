...
module Crypto::Mnemonic
  class Mnemonic
    ...
    private def getRandom(bits) : Array(UInt32)
      r = (1..(bits / 32)).map { rand(bits) }
      hex = r.reduce("") { |res, s| res + s.to_s(16) }
      if hex.hexbytes? == nil
    ...
    def to_hex : String
      @seed.reduce("") { |res, s| res + s.to_s(16) }
    end
    ...
    def self.from_hex(hex : String) : Mnemonic
      Mnemonic.new(hex.split("").in_groups_of(8).reduce([] of UInt32) do |seed, group|
        seed.push(group.join("").to_u32(16))
      end)
    end
